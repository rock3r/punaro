...
container_stage="$container_snapshot_file.new"
ssh_pve "$PUNARO_PVE_SSH_TARGET" pct push "$PUNARO_PVE_CONTAINER_ID" "$remote_stage" "$container_stage"
ssh_pve "$PUNARO_PVE_SSH_TARGET" "pct exec $PUNARO_PVE_CONTAINER_ID -- /bin/sh -ceu 'chown punaro:punaro \"\$1\"; chmod 600 \"\$1\"; mv -f \"\$1\" \"\$2\"' sh '$container_stage' '$container_snapshot_file'"
