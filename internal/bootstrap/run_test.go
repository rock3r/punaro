...
func TestRunGrandfathersLegacyRollbackTarget(t *testing.T) {
	...
	case err := <-errCh:
		t.Fatalf("run exited early: %v", err)
	case <-time.After(2 * time.Second):
		t.Fatal("legacy rollback target did not start")
	}
	time.Sleep(80 * time.Millisecond)
	if recoveryOnly(t, dir) {
		t.Fatal("legacy rollback target entered recovery-only")
	}
	cancel()
	if err := <-errCh; err != nil {
		t.Fatal(err)
...
func TestUpdateSucceedsAfterKeysDirectoryNode(t *testing.T) {
	dir := privateDir(t)
	writeNonFileMarker(t, filepath.Join(dir, directoryKeysFile))
	origin := newSignedOrigin(t, originSpec{payload: testArtifact, goos: runtime.GOOS, goarch: runtime.GOARCH})
	if _, err := Update(Request{
...
func TestUpdateClearsRecoveryDirectoryNode(t *testing.T) {
	...
	if _, err := os.Lstat(filepath.Join(dir, recoveryFile)); !os.IsNotExist(err) {
		t.Fatal("recovery directory node survived a signed update")
	}
}
