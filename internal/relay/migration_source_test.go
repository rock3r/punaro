...
func TestPreparedParentV3RoleOnlyMigrationSourcePreservesManifestIdentity(t *testing.T) {
	...
	if _, err := db.ExecContext(ctx, `DROP TABLE pending_capacity; DROP TABLE direct_message_idempotency; DROP TABLE message_from_roles; DROP TABLE direct_conversations; DROP TABLE rate_buckets; DROP TABLE role_profile_idempotency; DROP TABLE role_profiles; DROP TABLE conversation_control_idempotency; DROP TABLE conversation_controls`); err != nil {
		_ = db.Close()
		t.Fatal(err)
	}
	if err := db.Close(); err != nil {
...
func TestInspectMigrationSourceAcceptsPreparedParentWithoutRateBuckets(t *testing.T) {
	...
	if _, err := db.ExecContext(ctx, `DROP TABLE pending_capacity; DROP TABLE direct_message_idempotency; DROP TABLE message_from_roles; DROP TABLE direct_conversations; DROP TABLE rate_buckets`); err != nil {
		_ = db.Close()
		t.Fatal(err)
	}
	if err := db.Close(); err != nil {
...
func TestInspectMigrationSourceAcceptsPreparedParentWithoutDirectMessages(t *testing.T) {
	...
	if _, err := db.ExecContext(ctx, `DROP TABLE pending_capacity; DROP TABLE direct_message_idempotency; DROP TABLE message_from_roles; DROP TABLE direct_conversations`); err != nil {
		_ = db.Close()
		t.Fatal(err)
	}
	if err := db.Close(); err != nil {
...
func TestInspectMigrationSourceCarriesRateBucketsThroughCurrentCutoverSurface(t *testing.T) {
	...
	inspected, err := InspectMigrationSource(ctx, path)
	if err != nil || inspected.Version != 7 || inspected.Counts.RateBuckets != 2 || inspected.Counts.PendingCapacity != 2 {
...
func TestInspectMigrationSourceAcceptsRoleRecipientPendingCapacity(t *testing.T) {
	...
	inspected, err := InspectMigrationSource(ctx, path)
	if err != nil || inspected.Version != 7 || inspected.Counts.PendingCapacity != 2 {
