...
func TestPreparedV1MigrationSourceRemainsRecoverable(t *testing.T) {
	...
	if _, err := db.ExecContext(ctx, `DROP TABLE pending_capacity; DROP TABLE rate_buckets; DROP TABLE role_profile_idempotency; DROP TABLE role_profiles; DROP TABLE conversation_control_idempotency; DROP TABLE conversation_controls; DROP TABLE role_bindings; DROP TABLE role_memberships; DROP TABLE roles`); err != nil {
		_ = db.Close()
		t.Fatal(err)
	}
...
func TestPreparedParentV3RoleOnlyMigrationSourcePreservesManifestIdentity(t *testing.T) {
	...
	if _, err := db.ExecContext(ctx, `DROP TABLE pending_capacity; DROP TABLE rate_buckets; DROP TABLE role_profile_idempotency; DROP TABLE role_profiles; DROP TABLE conversation_control_idempotency; DROP TABLE conversation_controls`); err != nil {
		_ = db.Close()
		t.Fatal(err)
	}
	if err := db.Close(); err != nil {
...
func TestInspectMigrationSourceAcceptsPreparedParentWithoutRateBuckets(t *testing.T) {
	...
	if _, err := db.ExecContext(ctx, `DROP TABLE pending_capacity; DROP TABLE rate_buckets`); err != nil {
		_ = db.Close()
		t.Fatal(err)
	}
	if err := db.Close(); err != nil {
	...
	capacityBatch, err := ReadMigrationSourceBatch(ctx, path, "mail_pending_capacity", "", 10)
	if err != nil || len(capacityBatch.Rows) != 0 || !capacityBatch.Done {
		t.Fatalf("parent pending-capacity batch=%#v err=%v", capacityBatch, err)
	}
}

func TestInspectMigrationSourceAcceptsPreparedParentWithoutPendingCapacity(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	now := time.Date(2026, time.August, 18, 18, 15, 0, 0, time.UTC)
	path := filepath.Join(t.TempDir(), "relay.db")
	store, err := Open(path)
	if err != nil {
		t.Fatal(err)
	...
	if _, err := db.ExecContext(ctx, `DROP TABLE pending_capacity`); err != nil {
		_ = db.Close()
		t.Fatal(err)
	}
	if err := db.Close(); err != nil {
	...
	if err != nil || inspected.Version != 5 || inspected.Phase != MigrationSourceActive {
		t.Fatalf("parent without pending_capacity inspect=%#v err=%v", inspected, err)
	}
	...
	if err != nil || len(batch.Rows) != 0 || !batch.Done {
		t.Fatalf("parent pending-capacity batch=%#v err=%v", batch, err)
	}
}
...
func TestInspectMigrationSourceCarriesRateBucketsThroughCurrentCutoverSurface(t *testing.T) {
	...
	inspected, err := InspectMigrationSource(ctx, path)
	if err != nil || inspected.Version != 6 || inspected.Counts.RateBuckets != 2 || inspected.Counts.PendingCapacity != 2 {
