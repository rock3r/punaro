...
AS $function$ BEGIN RETURN NEW; END $function$`); err != nil {
	...
	for _, drift := range []struct {
		...
		{name: "project broad update privilege", apply: `GRANT UPDATE ON relay.projects TO punaro_app`, restore: `REVOKE UPDATE ON relay.projects FROM punaro_app`},
		{name: "project broad insert privilege", apply: `GRANT INSERT ON relay.projects TO punaro_app`, restore: `REVOKE INSERT ON relay.projects FROM punaro_app`},
