...
BEGIN
    ...
    UNION ALL
    SELECT fence.timeline_id,CASE WHEN change.operation<>'delete' AND memory_item.state='active' AND memory_revision.item_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM brain.memory_quarantines AS quarantine WHERE quarantine.item_id=change.item_id AND (quarantine.released_at IS NULL OR change.revision<>memory_item.current_revision)) THEN change.item_id END,CASE WHEN change.operation<>'delete' AND memory_item.state='active' AND memory_revision.item_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM brain.memory_quarantines AS quarantine WHERE quarantine.item_id=change.item_id AND (quarantine.released_at IS NULL OR change.revision<>memory_item.current_revision)) THEN change.revision END,change.change_sequence,false
