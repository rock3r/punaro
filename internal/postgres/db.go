    ...
    AND EXISTS (
        ...
        WHERE conrelid = 'audit.events'::regclass AND conname = 'events_action_check'
          ...
          AND (($1 = 3 AND pg_get_expr(conbin, conrelid) = '(action = ANY (ARRAY[''principal.create''::text, ''project.create''::text, ''grant.create''::text, ''grant.delete''::text, ''job.enqueue''::text, ''job.complete''::text, ''job.retry''::text, ''job.fail''::text, ''owner.bootstrap''::text, ''enrollment.create''::text, ''enrollment.redeem''::text, ''credential.rotate''::text, ''credential.revoke''::text, ''legacy.register''::text, ''legacy.exchange''::text, ''legacy.retire''::text, ''legacy.disable''::text]))')
            ...
            OR ($1 >= 18 AND pg_get_expr(conbin, conrelid) = '(action = ANY (ARRAY[''principal.create''::text, ''project.create''::text, ''grant.create''::text, ''grant.delete''::text, ''job.enqueue''::text, ''job.complete''::text, ''job.retry''::text, ''job.fail''::text, ''owner.bootstrap''::text, ''enrollment.create''::text, ''enrollment.redeem''::text, ''credential.rotate''::text, ''credential.revoke''::text, ''legacy.register''::text, ''legacy.exchange''::text, ''legacy.retire''::text, ''legacy.disable''::text, ''project.identity.attach''::text, ''project.merge.preview''::text, ''project.merge''::text, ''memory.create''::text, ''memory.evidence_create''::text, ''memory.update''::text, ''memory.archive''::text, ''memory.restore''::text, ''memory.delete''::text, ''memory.secret_exception.create''::text, ''memory.secret_exception.revoke''::text, ''memory.secret_rescan''::text, ''memory.quarantine''::text, ''memory.quarantine_release''::text, ''memory.proposal.create''::text, ''memory.proposal.approve''::text, ''memory.proposal.reject''::text, ''memory.proposal.expire''::text, ''memory.proposal.prune''::text]))'))
    )
    AND EXISTS (
        SELECT 1 FROM pg_constraint
