...
func (a *Administration) AbortMailCutover(ctx context.Context, actorPrincipalID, epochID, sourceFingerprint string) error {
	...
	if phase != MailCutoverAborted {
		for _, table := range []string{"mail_request_nonces", "mail_conversation_control_idempotency", "mail_conversation_controls", "mail_conversation_idempotency", "mail_message_idempotency", "mail_recipient_cursors", "mail_deliveries", "mail_messages", "mail_role_bindings", "mail_role_memberships", "mail_memberships", "mail_roles", "mail_conversations", "mail_endpoints"} {
    ...
    SELECT NOT EXISTS (SELECT * FROM expected_constraints EXCEPT SELECT * FROM actual_constraints)
       ...
       AND (SELECT bool_and(CASE con.conname
		...
		END
		   ...
		   WHEN 'mail_cutover_checkpoints_table_name_check' THEN pg_get_expr(con.conbin,con.conrelid)=CASE WHEN $1 >= 42 THEN '(table_name = ANY (ARRAY[''mail_endpoints''::text, ''mail_conversations''::text, ''mail_memberships''::text, ''mail_roles''::text, ''mail_role_memberships''::text, ''mail_role_bindings''::text, ''mail_messages''::text, ''mail_deliveries''::text, ''mail_recipient_cursors''::text, ''mail_message_idempotency''::text, ''mail_conversation_idempotency''::text, ''mail_conversation_controls''::text, ''mail_conversation_control_idempotency''::text, ''mail_request_nonces''::text]))' WHEN $1 >= 40 THEN '(table_name = ANY (ARRAY[''mail_endpoints''::text, ''mail_conversations''::text, ''mail_memberships''::text, ''mail_roles''::text, ''mail_role_memberships''::text, ''mail_role_bindings''::text, ''mail_messages''::text, ''mail_deliveries''::text, ''mail_recipient_cursors''::text, ''mail_message_idempotency''::text, ''mail_conversation_idempotency''::text, ''mail_request_nonces''::text]))' ELSE '(table_name = ANY (ARRAY[''mail_endpoints''::text, ''mail_conversations''::text, ''mail_memberships''::text, ''mail_messages''::text, ''mail_deliveries''::text, ''mail_recipient_cursors''::text, ''mail_message_idempotency''::text, ''mail_conversation_idempotency''::text, ''mail_request_nonces''::text]))' END
