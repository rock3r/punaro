...
func canonicalMailCutoverManifest(request MailCutoverRequest) ([]byte, error) {
	...
	counts := []int64{manifest.Counts.Endpoints, manifest.Counts.Conversations, manifest.Counts.Memberships, manifest.Counts.Roles, manifest.Counts.RoleMemberships, manifest.Counts.RoleBindings, manifest.Counts.Messages, manifest.Counts.Deliveries, manifest.Counts.RecipientCursors, manifest.Counts.MessageIdempotency, manifest.Counts.ConversationIdempotency, manifest.Counts.ControlEvents, manifest.Counts.ControlIdempotency, manifest.Counts.RequestNonces, manifest.Counts.RoleProfiles, manifest.Counts.RoleProfileIdempotency, manifest.Counts.RateBuckets}
	hashes := []string{manifest.TableSHA256.Endpoints, manifest.TableSHA256.Conversations, manifest.TableSHA256.Memberships, manifest.TableSHA256.Roles, manifest.TableSHA256.RoleMemberships, manifest.TableSHA256.RoleBindings, manifest.TableSHA256.Messages, manifest.TableSHA256.Deliveries, manifest.TableSHA256.RecipientCursors, manifest.TableSHA256.MessageIdempotency, manifest.TableSHA256.ConversationIdempotency, manifest.TableSHA256.ControlEvents, manifest.TableSHA256.ControlIdempotency, manifest.TableSHA256.RequestNonces, manifest.TableSHA256.RoleProfiles, manifest.TableSHA256.RoleProfileIdempotency, manifest.TableSHA256.RateBuckets}
	if (manifest.Version < 1 || manifest.Version > 5) || manifest.SourceID != request.SourceID || manifest.Phase != relay.MigrationSourcePrepared || manifest.EpochID != request.EpochID || manifest.TargetIdentity != request.TargetIdentity || manifest.Fingerprint != request.SourceFingerprint {
	...
	for index, hash := range hashes {
		if ((manifest.Version == 1 && index >= 3 && index <= 5) || ((manifest.Version <= 2 || parentRoleOnlyV3) && index >= 11 && index <= 12) || (manifest.Version < 4 && index >= 14) || (manifest.Version < 5 && index >= 16)) && hash == "" {
			continue
		}
