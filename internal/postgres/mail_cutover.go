...
func canonicalMailCutoverManifest(request MailCutoverRequest) ([]byte, error) {
	...
	counts := []int64{manifest.Counts.Endpoints, manifest.Counts.Conversations, manifest.Counts.Memberships, manifest.Counts.Roles, manifest.Counts.RoleMemberships, manifest.Counts.RoleBindings, manifest.Counts.Messages, manifest.Counts.Deliveries, manifest.Counts.RecipientCursors, manifest.Counts.MessageIdempotency, manifest.Counts.ConversationIdempotency, manifest.Counts.ControlEvents, manifest.Counts.ControlIdempotency, manifest.Counts.RequestNonces, manifest.Counts.RoleProfiles, manifest.Counts.RoleProfileIdempotency}
	hashes := []string{manifest.TableSHA256.Endpoints, manifest.TableSHA256.Conversations, manifest.TableSHA256.Memberships, manifest.TableSHA256.Roles, manifest.TableSHA256.RoleMemberships, manifest.TableSHA256.RoleBindings, manifest.TableSHA256.Messages, manifest.TableSHA256.Deliveries, manifest.TableSHA256.RecipientCursors, manifest.TableSHA256.MessageIdempotency, manifest.TableSHA256.ConversationIdempotency, manifest.TableSHA256.ControlEvents, manifest.TableSHA256.ControlIdempotency, manifest.TableSHA256.RequestNonces, manifest.TableSHA256.RoleProfiles, manifest.TableSHA256.RoleProfileIdempotency}
	if (manifest.Version < 1 || manifest.Version > 4) || manifest.SourceID != request.SourceID || manifest.Phase != relay.MigrationSourcePrepared || manifest.EpochID != request.EpochID || manifest.TargetIdentity != request.TargetIdentity || manifest.Fingerprint != request.SourceFingerprint {
	...
	for index, hash := range hashes {
		if ((manifest.Version == 1 && index >= 3 && index <= 5) || ((manifest.Version <= 2 || parentRoleOnlyV3) && index >= 11 && index <= 12) || (manifest.Version < 4 && index >= 14)) && hash == "" {
			continue
		}
...
func (a *Administration) AbortMailCutover(ctx context.Context, actorPrincipalID, epochID, sourceFingerprint string) error {
	...
	if phase != MailCutoverAborted {
		for _, table := range []string{"mail_request_nonces", "mail_role_profile_idempotency", "mail_role_profiles", "mail_conversation_control_idempotency", "mail_conversation_controls", "mail_conversation_idempotency", "mail_message_idempotency", "mail_recipient_cursors", "mail_deliveries", "mail_messages", "mail_role_bindings", "mail_role_memberships", "mail_memberships", "mail_roles", "mail_conversations", "mail_endpoints"} {
    ...
    SELECT NOT EXISTS (SELECT * FROM expected_constraints EXCEPT SELECT * FROM actual_constraints)
       ...
       AND (SELECT bool_and(CASE con.conname
		...
		END
		   ...
		   WHEN 'mail_cutover_checkpoints_table_name_check' THEN pg_get_expr(con.conbin,con.conrelid)=CASE WHEN $1 >= 45 THEN '(table_name = ANY (ARRAY[''mail_endpoints''::text, ''mail_conversations''::text, ''mail_memberships''::text, ''mail_roles''::text, ''mail_role_memberships''::text, ''mail_role_bindings''::text, ''mail_messages''::text, ''mail_deliveries''::text, ''mail_recipient_cursors''::text, ''mail_message_idempotency''::text, ''mail_conversation_idempotency''::text, ''mail_conversation_controls''::text, ''mail_conversation_control_idempotency''::text, ''mail_request_nonces''::text, ''mail_role_profiles''::text, ''mail_role_profile_idempotency''::text]))' WHEN $1 >= 42 THEN '(table_name = ANY (ARRAY[''mail_endpoints''::text, ''mail_conversations''::text, ''mail_memberships''::text, ''mail_roles''::text, ''mail_role_memberships''::text, ''mail_role_bindings''::text, ''mail_messages''::text, ''mail_deliveries''::text, ''mail_recipient_cursors''::text, ''mail_message_idempotency''::text, ''mail_conversation_idempotency''::text, ''mail_conversation_controls''::text, ''mail_conversation_control_idempotency''::text, ''mail_request_nonces''::text]))' WHEN $1 >= 40 THEN '(table_name = ANY (ARRAY[''mail_endpoints''::text, ''mail_conversations''::text, ''mail_memberships''::text, ''mail_roles''::text, ''mail_role_memberships''::text, ''mail_role_bindings''::text, ''mail_messages''::text, ''mail_deliveries''::text, ''mail_recipient_cursors''::text, ''mail_message_idempotency''::text, ''mail_conversation_idempotency''::text, ''mail_request_nonces''::text]))' ELSE '(table_name = ANY (ARRAY[''mail_endpoints''::text, ''mail_conversations''::text, ''mail_memberships''::text, ''mail_messages''::text, ''mail_deliveries''::text, ''mail_recipient_cursors''::text, ''mail_message_idempotency''::text, ''mail_conversation_idempotency''::text, ''mail_request_nonces''::text]))' END
