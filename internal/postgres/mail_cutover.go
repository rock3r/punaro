...
func canonicalMailCutoverManifest(request MailCutoverRequest) ([]byte, error) {
	...
	if (manifest.Version < 1 || manifest.Version > 6) || manifest.SourceID != request.SourceID || manifest.Phase != relay.MigrationSourcePrepared || manifest.EpochID != request.EpochID || manifest.TargetIdentity != request.TargetIdentity || manifest.Fingerprint != request.SourceFingerprint {
	...
	for index, hash := range hashes {
		if ((manifest.Version == 1 && index >= 3 && index <= 5) || ((manifest.Version <= 2 || parentRoleOnlyV3) && index >= 11 && index <= 12) || (manifest.Version < 4 && index >= 14) || (manifest.Version < 5 && index >= 16) || (manifest.Version < 6 && index >= 17)) && hash == "" {
			continue
		}
