...
// relayRateLimitControlsAvailable confirms the durable token-bucket table is
...
WITH objects AS (
    ...
    SELECT count(*)=4
       AND bool_and((attribute.attrelid,attribute.attname,attribute.atttypid,attribute.attnotnull) IN (
           (buckets_oid,'kind','text'::regtype,true),(buckets_oid,'bucket_key','text'::regtype,true),
           (buckets_oid,'tokens','int8'::regtype,true),(buckets_oid,'updated_at','timestamptz'::regtype,true))) AS exact
    ...
    SELECT expected.* FROM objects, LATERAL (VALUES
        ...
        (buckets_oid,'mail_rate_buckets_kind_check','c'::"char",ARRAY[1]::smallint[],'(kind = ANY (ARRAY[''sender''::text, ''conversation''::text]))'),
        ...
        (buckets_oid,'mail_rate_buckets_tokens_check','c'::"char",ARRAY[3]::smallint[],'(tokens >= 0)')
    ...
    SELECT has_table_privilege('punaro_app',buckets_oid,'SELECT,INSERT')
       AND NOT has_table_privilege('punaro_app',buckets_oid,'DELETE,TRUNCATE,REFERENCES,TRIGGER')
       AND has_column_privilege('punaro_app',buckets_oid,'tokens','UPDATE')
       AND has_column_privilege('punaro_app',buckets_oid,'updated_at','UPDATE')
       AND NOT has_column_privilege('punaro_app',buckets_oid,'kind','UPDATE')
