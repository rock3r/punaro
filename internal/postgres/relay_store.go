...
func (d *Database) SendDirectMessage(input relay.DirectMessageInput) (relay.Message, bool, error) {
	...
	if err := d.reservePendingCapacity(tx, []string{"\x1erole:" + input.ToRole}, int64(len(input.Body))); err != nil {
		return relay.Message{}, false, err
	}
...
func (d *Database) ApplyControl(input relay.ControlInput) (relay.ControlEvent, bool, error) {
	...
	if input.Operation == relay.ControlUpsertMember && err == nil && previous&relay.CapReceive != 0 && input.Member.Capabilities&relay.CapReceive == 0 {
		if err := postgresRetirePendingDeliveries(tx, input.Member.Endpoint, input.ConversationID, input.Now); err != nil {
			return relay.ControlEvent{}, false, err
		}
...
func (d *Database) AppendMessage(input relay.AppendInput) (relay.Message, bool, error) {
	...
	targets, err := postgresDeliveryTargets(tx, input.ConversationID, input.FromEndpoint, input.TargetRole, rolesAvailable)
	if err != nil {
		return relay.Message{}, false, err
	}
	if err := d.reservePendingCapacity(tx, targets, int64(len(input.Body))); err != nil {
		return relay.Message{}, false, err
	}
...
func (d *Database) AckDelivery(machineID, endpoint, deliveryID, token string, generation int64, now time.Time) error {
	...
	charge, err := postgresPendingDeliveryCharge(tx, deliveryID)
	if err != nil {
		return err
	}
	...
	if affected == 1 {
		if err := postgresReleasePendingCharges(tx, []relay.PendingCharge{charge}); err != nil {
			return err
		}
	}
