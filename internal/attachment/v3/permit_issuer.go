...
func validatePermitRequest(r PermitRequest) error {
	if r.RequestID == [16]byte{} || r.HolderDeviceID == [16]byte{} || r.TransferID == [16]byte{} || r.ConversationID == [16]byte{} || r.SenderDeviceID == [16]byte{} || r.RecipientDeviceID == [16]byte{} || r.MembershipCommitment == [32]byte{} || r.StagedManifestCommitment == [32]byte{} || r.HolderGeneration == 0 || r.SenderGeneration == 0 || r.RecipientGeneration == 0 || r.HolderRole < permitHolderSender || r.HolderRole > permitHolderRecipient || r.Operation < permitOperationSourceInit || r.Operation > permitOperationCancel || r.IssuedAt > math.MaxInt64 || r.ExpiresAt > math.MaxInt64 || r.ExpiresAt <= r.IssuedAt || r.ExpiresAt-r.IssuedAt > 30 || r.MaxBytes == 0 || r.MaxBytes > maxPermitCiphertextBytes || r.MaxChunks == 0 || r.MaxChunks > 4096 || r.MaxOperations == 0 || r.MaxOperations > 4096 {
