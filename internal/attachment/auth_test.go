...
func TestEd25519AuthenticatorRetainsNonceUntilFutureSignatureExpires(t *testing.T) {
	public, private, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	...
	request := signedRequest(t, private, signedAt, "future-nonce", []byte("ciphertext"))
	if _, err := auth.Authenticate(request.Context(), request); err != nil {
		t.Fatalf("future-dated request rejected: %v", err)
	}
	clock = clock.Add(authWindow + time.Second)
	replay := signedRequest(t, private, signedAt, "future-nonce", []byte("ciphertext"))
	if _, err := auth.Authenticate(replay.Context(), replay); err == nil {
		t.Fatal("replay was accepted before the future-dated signature expired")
	}
}
