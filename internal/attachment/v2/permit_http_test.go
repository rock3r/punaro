...
func TestPermitHTTPHandlerRejectsRequestWithoutRouteAdmission(t *testing.T) {
	issuerPublic, issuerPrivate, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	...
	handler, err := NewPermitHTTPHandler(issuer, staticPermitIssuanceAuthorityProvider{authority: authority}, permitRequestAuthorizerFunc(func(context.Context, PermitRequest) error { return errors.New("wrong machine") }), func() time.Time { return clock })
	if err != nil {
		t.Fatal(err)
	}
	httpRequest := httptest.NewRequestWithContext(context.Background(), http.MethodPost, "/v2/permits", bytes.NewReader(rawRequest))
