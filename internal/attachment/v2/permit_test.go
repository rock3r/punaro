...
func TestPermitCanonicalSignatureAndFreshIssuerBinding(t *testing.T) {
	...
	resolver := permitAuthorityStub{keyID: permit.IssuerKeyID, key: issuerPublic}
	...
	if err := VerifyPermit(permit, permitAuthorityStub{keyID: permit.IssuerKeyID, key: issuerPublic, reject: true}, clock); err == nil {
...
func (s permitAuthorityStub) ValidatePermitAuthority(permit Permit, _ time.Time) (ed25519.PublicKey, error) {
	if s.reject || permit.IssuerKeyID != s.keyID {
...
func samplePermit() Permit {
	return Permit{
		...
		TransferID: bytes16(5), ConversationID: bytes16(6), SenderDeviceID: bytes16(4), SenderGeneration: 1, RecipientDeviceID: bytes16(7),
