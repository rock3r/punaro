...
"The full Punaro gate" below means the AGENTS.md quality gate: `make test`,
`make test-race`, `make staticcheck`, `make security`, `make lint`.

## CI classification checklist

Treat as **branch-related** when logs clearly indicate a regression caused by the PR branch:

- Compile/vet/lint failures in files or packages touched by the branch
- Deterministic unit/integration test failures in changed areas
- `staticcheck`, `gosec`, or `golangci-lint` findings introduced by the latest push
- Race-detector failures (`make test-race`) in code the branch touches
- Build script/Makefile/config changes in the PR causing a deterministic failure

Treat as **likely flaky or unrelated** when evidence points to transient or external issues:

- DNS/network/proxy timeout errors while fetching Go modules
- Runner image provisioning or startup failures
- GitHub Actions infrastructure/service outages
- Non-deterministic failures in concurrency or timing-sensitive tests (especially e2e
  suites involving Docker, Postgres, or the relay lifecycle)
...
2. If there are failed checks:
   ...
   - If branch-related: **start fixing locally right away** — do not wait for other
     checks or bots to finish. But **do not push yet** — continue to steps 3 and 4 first.
...
3. As bots or human reviewers post new comments while you are mid-fix, **incorporate
   their fixes into the same local batch**. Do not wait idle — keep fixing as issues come
   in. The only thing to wait for is pushing: do not push until all bots and checks have
   finished running so you can be sure you have collected everything.
4. **Push once** only when: all fixes are done AND all review bots are done AND the full
   Punaro gate is green.
5. After the push (and before merging), **resolve every review bot comment thread on
   GitHub**: if the issue was fixed, resolve the thread; if it does not apply, post a
   short reply explaining why and then resolve the thread. The PR must have no open bot
   threads at merge time.
6. If flaky reruns for the same SHA reach the configured limit (default 3): stop and report.

> **Cost rule**: every push triggers review-bot runs. Batch all local fixes — CI
> failures, bot comments, human review comments — into a single commit before pushing.
...
- After fixing, run the full Punaro gate to confirm no regressions.
...
- The full Punaro gate fails after an attempted fix and the cause is not obvious.
