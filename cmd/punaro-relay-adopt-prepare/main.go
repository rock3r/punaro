...
func run(args []string, stdout, stderr io.Writer) int {
	...
	var relayDB, keeper, nonKeeper, nonKeeperName, dropRole string
	var yes bool
	...
	// Required and must equal TelegramCodexRole so the adopt runbook flag is accepted
	// without letting the operator drop a different role.
	flags.StringVar(&dropRole, "drop-role", "", "must be role/telegram-codex")
	flags.BoolVar(&yes, "yes", false, "required confirmation to mutate the local relay store")
	if err := flags.Parse(args); err != nil {
		return 2
	}
	if flags.NArg() != 0 || strings.TrimSpace(relayDB) == "" || strings.TrimSpace(keeper) == "" || strings.TrimSpace(nonKeeper) == "" || strings.TrimSpace(nonKeeperName) == "" || dropRole != relay.TelegramCodexRole || !yes {
		_, _ = fmt.Fprintln(stderr, "usage: punaro-relay-adopt-prepare --relay-db PATH --keeper ID --non-keeper ID --non-keeper-name NAME --drop-role role/telegram-codex --yes")
		return 2
	}
	info, err := os.Stat(relayDB)
	if err != nil {
		if os.IsNotExist(err) {
			_, _ = fmt.Fprintln(stderr, "punaro-relay-adopt-prepare failed: relay database is missing")
			return 1
		}
