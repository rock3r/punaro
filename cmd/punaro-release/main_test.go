...
func TestBuildFactsBindsPluginSkillsGeneratedComposeAndMigrations(t *testing.T) {
	...
	facts, err := buildFacts([]string{"--release", "v0.1.0-alpha.13", "--plugin-root", root})
	if err != nil || facts.Release != "v0.1.0-alpha.13" || facts.ComposeSHA256 != operator.ComposeManifestSHA256() || len(facts.MigrationManifestSHA256) != 64 || len(facts.SkillSetSHA256) != 64 || len(facts.PluginRuntimeSHA256) != 64 {
