...
counters, and migration 49 adds terminal delivery retention. After migration
48, `punaro relay reconcile-capacity
...
repair path. After migration 49, `punaro relay terminal list --directory DIR`
pages content-free expired and revoked delivery metadata, and `punaro relay
terminal maintain --directory DIR --yes` runs one bounded expiry and prune
batch. Dead-letter inspection is host-local; it is not an agent API.
