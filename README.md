...
| `PUNARO_REMOTE_MCP_TOKEN_VALIDATION_ENABLED` | `false` | Enables strict JWT validation for remote MCP bearer tokens. Requires the enabled metadata gate, an issuer listed in `PUNARO_REMOTE_MCP_AUTHORIZATION_SERVERS`, `PUNARO_REMOTE_MCP_JWKS_URL` over HTTPS, and subject bindings. Tokens must be signed, unexpired, and audience-bound to the canonical MCP resource. A verified, bound token still reaches no MCP transport or tools in this slice. |
| `PUNARO_REMOTE_MCP_SUBJECT_BINDINGS_JSON` | unset | Required when remote MCP token validation is enabled: a bounded JSON array of unique `{"subject":"...","principal_id":"existing-principal-uuid"}` entries. It binds an OAuth subject to an existing Punaro principal; it does not accept a client-supplied project claim or grant any project capability. A later transport must enforce both the token scope and the authoritative server-side project grant for that bound principal. |
