...
only PostgreSQL and `punarod` by default, pins both image inputs, keeps
PostgreSQL private, and accepts credentials only through read-only secret
files. It must be initialized with the host-local `punaro` workflow before the
ordinary application service is started.
