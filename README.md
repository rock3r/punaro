...
| `PUNARO_REMOTE_MCP_TOKEN_VALIDATION_ENABLED` | `false` | Enables strict JWT validation for remote MCP bearer tokens. Requires the enabled metadata gate, an issuer listed in `PUNARO_REMOTE_MCP_AUTHORIZATION_SERVERS`, and `PUNARO_REMOTE_MCP_JWKS_URL` over HTTPS. Tokens must be signed, unexpired, and audience-bound to the canonical MCP resource. A verified token still reaches no MCP transport or tools in this slice. |
